...
# ----------------------------------------
from Chefboost.chefboost.commons import functions
from Chefboost.chefboost.training import Preprocess
...
def createBranch(config, current_class, subdataset, numericColumn, branch_index, winner_name, winner_index, root,
                 parents, file, dataset_features, num_of_instances, metric, tree_id=0, main_process_id=None):
    custom_rules = []
    ...
    if terminateBuilding == True:  # check decision is made
        ...
        if enableParallelism != True:
            ...
            # parallel
            sample_rule = {"current_level": root + 1, "leaf_id": leaf_id, "parents": parents, "rule": decision_rule,
                           "feature_idx": winner_index, "feature_name": winner_name, "instances": num_of_instances,
                           "metric": 0, "return_statement": 1, "tree_id": tree_id}
